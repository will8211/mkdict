...
class Cipher(object):
    ...
    def __init__(self, key, byte_order="big", P_array=PI_P_ARRAY, S_boxes=PI_S_BOXES):
        if not 4 <= len(key) <= 56:
        ...
        P = [
            (p1 ^ k1, p2 ^ k2)
            for p1, p2, k1, k2 in zip(
                P_array[0::2], P_array[1::2], cyclic_key_u4_iter, cyclic_key_u4_iter
            )
        ]
        ...
        for i in range(len(P)):
            P[i] = L, R = encrypt(L, R, P, S1, S2, S3, S4,
    ...
    def encrypt_ecb(self, data):
        ...
        try:
            ...
            raise ValueError(
                "data is not a multiple of the block-size in length")
    ...
    def encrypt_cbc_cts(self, data, init_vector):
        ...
        P_L, P_R = u4_2_unpack(
            data[last_block_stop_i:] + bytes(8 - extra_bytes))
    ...
    def decrypt_cbc_cts(self, data, init_vector):
        ...
        L, R = decrypt(cipher_L, cipher_R, P, S1, S2,
                       S3, S4, u4_1_pack, u1_4_unpack)
    ...
    def encrypt_cfb(self, data, init_vector):
        ...
        for plain_L, plain_R in self._u4_2_iter_unpack(data[0:last_block_stop_i]):
            prev_cipher_L, prev_cipher_R = encrypt(
                prev_cipher_L, prev_cipher_R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack
            )
            prev_cipher_L ^= plain_L
            prev_cipher_R ^= plain_R
            yield u4_2_pack(prev_cipher_L, prev_cipher_R)
    ...
    def decrypt_cfb(self, data, init_vector):
        ...
        for cipher_L, cipher_R in self._u4_2_iter_unpack(data[0:last_block_stop_i]):
            prev_cipher_L, prev_cipher_R = encrypt(
                prev_cipher_L, prev_cipher_R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack
            )
    ...
    def encrypt_ofb(self, data, init_vector):
        ...
        for plain_L, plain_R in self._u4_2_iter_unpack(data[0:last_block_stop_i]):
            prev_L, prev_R = encrypt(
                prev_L, prev_R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack
            )
    ...
    def encrypt_ctr(self, data, counter):
        ...
        for (plain_L, plain_R), counter_n in zip(
            self._u4_2_iter_unpack(data[0:last_block_stop_i]), counter
        ):
            try:
            ...
            counter_L, counter_R = encrypt(
                counter_L, counter_R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack
            )
        ...
        if extra_bytes:
            ...
            yield bytes(
                b ^ n
                for b, n in zip(
                    data[last_block_stop_i:], u4_2_pack(counter_L, counter_R)
                )
            )
